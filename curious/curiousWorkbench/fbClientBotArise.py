...
class fbClientBotArise():
    ...
    def processEvent(self, event, fbid, recevied_message=''):
        try:
            ...
            self.logger.info("fbid : " + fbid)
            self.logger.info("recevied_message : " + recevied_message)
            ...
            for subscription in stateMachineSubscriptions:
                ...
                if process == True:
                    ...
                    if strNextEvent !='':
                        strMessageNew = self.processEvent(strNextEvent,fbid, '')
    ...
    def UpdateUserInfo(self, userID):
        ...
        if (status.ok):
            ...
            mp.track(fbid, "New_User",{'User_Gender':dictUserDetails["gender"],'User_Age':strToState,'':''})
    ...
    def actionGetModule(self, userID, recevied_message,strDictParams=""):
        try:
            ...
            if strDictParams.strip()!="":
                ...
                dictParams={"ID":1}
            dictParams={"ID":1}
            strModule= self.r_server.hget( "KEY_MODULE_"+str(dictParams["ID"]), "ID")
            dictModule = json.loads(strModule)
            strMessage = dictModule["Title"][:79]
            strVideoURL = ""
    ...
    def actionGetModuleCompletionMessage(self, userID, recevied_message, strDictParams=""):
        try:
            ...
            strMessage="Awesome, you have compelted this module"
            ...
            dictButtons =[]
            strLink = "http://www.walnutai.com/curiousWorkbench/fbABWCV/" + str(1)
            self.logger.info("heeree 40")
            dictButtons.append(self.getButton("web_url","Linkedin Share",surl=strLink,spayload=""))
            strDictButtons= json.dumps(dictButtons)
    ...
    def actionGetRoleDemandInfo(self, userID, recevied_message,strDictParams=""):
        try:
            ...
            strMessage = "Here are the top 5 Skills that are being asked for\r\n"
            intMax = 4
            intCount = 0
            strQuickReplies = ""
            strRoleInfoDesc = ""
            for role in roleInfo:
                if intCount <= intMax:
                    ...
                    if dictRoleInfo["Enabled"]=="TRUE":
                        strRoleInfoDesc += dictRoleInfo["Skill"] + " : " +str(dictRoleInfo["Percentage"]) + "\r\n"
                        if intCount != 0:
                            strQuickReplies += ","
                        strPayLoad = "SHOW_MODULE" + "-" + "ID" + "|" + "1"
                        strQuickReplies += "text" + ":" + str(dictRoleInfo["Skill"]) + ":" + strPayLoad
