...
absRolesFolderPath = ""
rolesFile = "product_manager.csv"
filePath = absRolesFolderPath + rolesFile

file = open(filePath,'rb')
lineReader = csv.DictReader(file, delimiter = ",")
cur.execute("delete from curiousWorkbench_roledemandinfo where ID !=0")
Percentage=0
for line in lineReader:
    ...
    strSQL += "Values"
    strSQL +="('Product_Manager','SFO','1001','" + line['Role'].replace("_"," ") + "','" + line['Role'] + "'," + str(Percentage) + ",'TRUE'," + line['Demand_Count'] + "," + line['World_Count'] + ") ;"
    print strSQL
    cur.execute(strSQL)
    db.commit()
