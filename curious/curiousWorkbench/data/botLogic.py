...
class botLogic():
    ...
    def actionGetReco(self, userID, recevied_message,strDictParams=""):
        try:
            ...
            self.logger.info("2")
            arrAltMessage = []
            #--------------Get a random content based on some filters-------------------------------------------------------------
            contentLibraryObj = ContentLibrary.objects.filter(Rating__gte=0)
            ...
            if contentLibrary.Message_Type =="External_Content":
                if contentLibrary.Type == "YouTube":
                    dictParams = self.getContentExternalYouTube( userID,contentLibrary)
                elif contentLibrary.Type =="URL":
                    dictParams = self.getContentExternalURL( userID,contentLibrary)
            elif contentLibrary.Message_Type == "UGC":
                if contentLibrary.Type == "Text":
                    dictParams = self.getContentUGCText( userID,contentLibrary)
                elif contentLibrary.Type == "Video":
                    dictParams = self.getContentUGCVideo( userID,contentLibrary)
                elif contentLibrary.Type == "Image":
                    dictParams = self.getContentUGCImage( userID,contentLibrary)
            ...
            #---------------Add Challenge----------------------------------------------------------------------------
            dictParamsChallenge = self.getChallengeMessage( userID,contentLibrary.ID)
    ...
    def getContentUGCText(self, userID, contentLibrary):
        ...
        if contentLibrary.Tags is not None:
            strMessage += str(contentLibrary.Tags)
        strMessage += "\r\n"
        strMessage += "❄️️ "
    ...
    def getContentUGCImage(self, userID, contentLibrary):
        ...
        strMessage = str(contentLibrary.Text)
        strImage = contentLibrary.ImageURL
        strSubTitleInfo = ""
        strImage = ""
        strVideoURL = contentLibrary.LinkURL
    ...
    def getChallengeMessage(self, userID, contentID):
        strMessageType = "Text"
        strMessage = ""
        strImage = ""
        strSubTitleInfo = ""
    ...
    def actionShareChallenge(self, userID, recevied_message,strDictParams=""):
        try:
            ...
            if strDictParams.strip()!="":
                dictParams = json.loads(strDictParams)
                if "CHALLENGE_ID" in dictParams:
                    userCreatedContentID = dictParams["CHALLENGE_ID"]

            strMessage1= "Copy and share the link below with your friends"

            strMessage2 = "Here is a challenge for you \r\n"
            strMessage2 += " https://m.me/TestArise?ref=" + str(userCreatedContentID)
            strMessageType = "Text"
            strVideoURL = ""
            strImage = ""
            strDictButtons =""
    ...
    def actionDeleteChallenge(self, userID, recevied_message,strDictParams=""):
        try:
            ...
            objChallenge = Challenge.objects.get(id=userCreatedContentID)
            objChallenge.delete()
    ...
    def actionTestChallenge(self, userID, recevied_message,strDictParams=""):
        try:
            ...
            strQuickReplies = "text" + ":" + "Share" + ":" + "SHARE_CHALLENGE-CHALLENGE_ID|" + str(challengeID)
            strQuickReplies += "," + "text" + ":" + "Delete" + ":" + "DELETE_CHALLENGE-CHALLENGE_ID|" + str(challengeID)
    ...
    def getMessageDict(self, strMessageType, strMessage, strImage, strSubTitleInfo, strImage1, strVideoURL, strLink, strDictButtons, strQuickReplies,strNotificationType="SILENT",strListContent="",strListButtons=""):
        dictRet={}
    ...
    def actionCheckChallengeAns(self, userID, recevied_message,strDictParams=""):
        try:
            ...
            self.logger.info("1")
            userStateObj = UserState.objects.get(pk=userID)
            objChallenge = Challenge.objects.get(pk=Challenge_ID)
            strUserRole =  userStateObj.UserRole
            if strDictParams !="":
                dictParams = json.loads(strDictParams)
                if "CHALLENGE_ID" in dictParams:
            ...
            if strAns == objChallenge.Correct_Answer:
                ...
                strMessage = "Yay 🎉🎉🎉🎉🎉 , Thats right !"
            else:
                objChallengeResultUser.IsCorrect == "N"
                strMessage = "🙁 Oops thats not right !\r\n"
                strMessage += str(objChallenge.Question_Text) + "\r\n"
                strMessage += str(objChallenge.Option_A) + "\r\n" + "\r\n"
                strMessage += "Here is how others did 👀"
            arrSummary=[]
            if strAns =="A":
            ...
            bar_chart = pygal.Bar(print_values=True,print_values_position='top',  print_labels=True,legend_at_bottom=True,include_x_axis=False,include_y_axis=False,show_y_guides=False,margin=50,)
            if objChallenge.Option_A is not None:
                bar_chart.add(objChallenge.Option_A, [objChallengeResultSummary.Option_A_Count])
            if objChallenge.Option_B is not None:
                bar_chart.add(objChallenge.Option_B, [objChallengeResultSummary.Option_B_Count] )
            if objChallenge.Option_C is not None:
                bar_chart.add(objChallenge.Option_C, [objChallengeResultSummary.Option_C_Count] )
            if objChallenge.Option_D is not None:
                bar_chart.add(objChallenge.Option_D, [objChallengeResultSummary.Option_D_Count] )
            if objChallenge.Option_E is not None:
            ...
            dictButtons =[]
            strLink= "https://m.me/TestArise?ref=" + str(challengeID)
            dictButtons.append(self.getButton("web_url","Next >>",surl=strLink,spayload=""))
            dictButtons.append(self.getButton("element_share","Challenge Friend",surl="",spayload=""))

            strDictButtons = json.dumps(dictButtons)

            dictParams = self.getMessageDict("Text",strMessage, "", "", "", "",self.configSettingsObj.webUrl, "","","SILENT")
            retArr.append(dictParams)
            ...
            strMessage = "_"
            strSubTitleInfo ="_"
            dictParams1 = self.getRecoButtons(recoPayload,strMessage,strSubTitleInfo,str(objChallenge.Content_ID))
            strQuickReplies = dictParams1["QuickReplies"]
            self.logger.info("8")
            dictParams = self.getMessageDict("Image","", chartURL, "", chartURL, "",self.configSettingsObj.webUrl, "",strQuickReplies,"SILENT")
            retArr.append(dictParams)
    ...
    def getSkillList(self,userID,recevied_message,strDictParams="",UserSkillOnlyFlag=False):
        try:
            ...
            ##self.logger.info(recevied_message + "---00--")
            if recevied_message !="":
                roleDemandInfoObj = RoleDemandInfo.objects.filter(Enabled="TRUE",Skill__icontains=recevied_message)
            # elif intRandIntUserRole < 88 and userStateObj.UserRole=="Product_Manager":
            #     roleDemandInfoObj = RoleDemandInfo.objects.filter(Enabled="TRUE",Skill__contains=recevied_message)
            else:
                roleDemandInfoObj = RoleDemandInfo.objects.exclude(Role="Product_Manager").filter(Enabled="TRUE")


            intNumberofRoles = roleDemandInfoObj.count()
            ##self.logger.info("records ---" + str(intNumberofRoles))
            strMessage = ""
